...
export const CostosVsPresupuestoChart = ({ data, facturacionData }: CostosVsPresupuestoChartProps) => {
    ...
    const option = useMemo(() => {
        ...
        if (facturacionRealData && facturacionRealData.length > 0) {
            legendItems.push('Facturación Real GCP');
        }
    ...
    return (
        <Card className="w-full">
            <CardHeader>
                <CardTitle>Costos vs Presupuesto GCP</CardTitle>
            </CardHeader>
            <CardContent>
                <div className="flex items-center justify-center gap-2 mb-2">
                    <Info className="w-5 h-5 text-blue-500 flex-shrink-0" />
                    <p className="text-xs text-muted-foreground">
                        Comparación mensual entre presupuesto asignado, costos reales, pronóstico y facturación real de Google Cloud.
                    </p>
                </div>
                {(!data || data.length === 0) ? (
