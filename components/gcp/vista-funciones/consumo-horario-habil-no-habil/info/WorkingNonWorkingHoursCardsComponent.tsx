...
interface WorkingNonWorkingHoursCardsComponentProps {
...
const getMetricConfig = (metricName: string) => {
    ...
    if (lowerName.includes('cpu')) return { icon: Cpu, unit: '%' };
    if (lowerName.includes('iops')) return { icon: HardDrive, unit: 'IOPS' };

    if (lowerName.includes('throughput')) return { icon: Activity, unit: 'B/s' };

    if (lowerName.includes('pps') || (lowerName.includes('network') && !lowerName.includes('throughput'))) {
        if (lowerName.includes('egress')) return { icon: ArrowUpRight, unit: 'pps' };
...
export const WorkingNonWorkingHoursCardsComponent = ({ data }: WorkingNonWorkingHoursCardsComponentProps) => {
