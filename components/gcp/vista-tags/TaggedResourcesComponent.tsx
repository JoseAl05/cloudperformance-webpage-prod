...
import { TagsDataTable } from './table/TagsDataTable'
...
export const TagsAnalysisComponent = ({ startDate, endDate }: TagsAnalysisComponentProps) => {
  ...
  const { data, isLoading, error } = useSWR<unknown>(
    `/api/gcp/bridge/mantenedor-tags/gcp/get_resource_tags?date_from=${startDateFormatted}&date_to=${endDateFormatted}`,
    fetcher
  )
    ...
    return (
      <div className="w-full min-w-0 px-4 py-10">
        <MessageCard
          ...
          description="Hubo un problema de conexión con el endpoint de tags."
          tone="error"
        />
      </div>
    )
    ...
    return (
      <div className="w-full min-w-0 px-4 py-6">
        <MessageCard
          icon={Info}
          title="Sin recursos"
          description="No se detectaron recursos en este rango de fechas."
          tone="info"
        />
      </div>
    )
    ...
    <div className="w-full min-w-0 px-4 py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 w-full">
        ...
        <Card className={`border-l-4 shadow-sm flex flex-col justify-between w-full ${recursosSinTags > 0 ? 'border-l-red-500' : 'border-l-green-500'}`}>
          <CardContent className="p-4 sm:p-5">
            <div className="flex justify-between items-start mb-3">
              <p className="text-xs sm:text-sm font-semibold text-gray-500 uppercase tracking-wide">Huérfanos (Sin Tags)</p>
              <ServerOff className={`h-5 w-5 opacity-80 ${recursosSinTags > 0 ? 'text-red-500' : 'text-green-500'}`} />
            </div>
            <p className={`text-2xl sm:text-3xl font-bold truncate ${recursosSinTags > 0 ? 'text-red-600' : 'text-green-600'}`}>
              {recursosSinTags}
            </p>
          </CardContent>
        </Card>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-3 border rounded-lg bg-blue-50/30 border-blue-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-800">Solo en GCP</p>
                ...
                <div className="flex items-center justify-between p-3 border rounded-lg bg-emerald-50/30 border-emerald-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Tienen tags en GCP y Locales</p>
                    </div>
                  </div>
                  <div className="text-right">
