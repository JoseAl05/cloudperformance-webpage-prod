...
export interface ResourceBillingGroup extends InstanceGroupsBilling {
    items: InstanceGroupsBilling[];
}
...
const SkuBreakdownTable = ({ data }: { data: InstanceGroupsBilling[] }) => {
    const aggregatedSkus = useMemo(() => {
        const skuMap = new Map<string, InstanceGroupsBilling>();
        data.forEach((item) => {
...
export const getSkuBillingColumns = (): DynamicColumn<InstanceGroupsBilling>[] => {
    return [
        {
            header: "SKU",
            accessorKey: "sku",
