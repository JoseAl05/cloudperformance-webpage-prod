...
import { Activity, HardDrive, Network, Clock, Cpu, MemoryStick } from 'lucide-react';
import { useMemo } from 'react';

interface CloudSqlMetricsCardComponentProps {
    data?: CloudSqlMetrics[];
}

const metricUnits: Record<string, string> = {
    active_connections: "Conexiones",
    ...
    memory_utilization: "%",
    storage_used_gb: "GB",
    db_queries_sec: "Queries/s",
    db_transactions_sec: "T/s"
};

const getMetricIcon = (metricName: string) => {
    if (metricName.includes('network')) return Network;
    if (metricName.includes('disk') || metricName.includes('storage')) return HardDrive;
    if (metricName.includes('cpu')) return Cpu;
    if (metricName.includes('uptime')) return Clock;
    if (metricName.includes('memory')) return MemoryStick;
    return Activity;
};
...
export const CloudSqlMetricsCardComponent = ({ data }: CloudSqlMetricsCardComponentProps) => {
