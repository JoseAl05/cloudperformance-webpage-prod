...
import { CloudSqlMetrics } from '@/interfaces/vista-cloudsql/cSqlInterfaces';


interface CloudSqlChartComponentProps {
    data: CloudSqlMetrics[];
}

const SingleMetricChart = ({ metric }: { metric: CloudSqlMetrics }) => {
    ...
    const option = useMemo(() => {
        ...
        if (metricName.includes('cpu_utilization') || metricName.includes('memory_utilization')) {
            ...
            axisLabelFormatter = (value: number) => `${formatGeneric(value)} conexiones`;
            tooltipFormatter = (v: number | null) => v != null ? `${Number(v).toFixed(2)} conexiones` : '-';
        }
        else if (metricName.includes('used_gb')) {
            yAxisName = `${metricName} (GB)`;
            axisLabelFormatter = (value: number) => `${value} GB`;
            tooltipFormatter = (v: number | null) => v != null ? `${Number(v).toFixed(2)} GB` : '-';
        }
...
export const CloudSqlChartComponent = ({ data }: CloudSqlChartComponentProps) => {
