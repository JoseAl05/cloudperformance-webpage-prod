...
export const HistoricalBillingTable = ({ data, isLoading, cloudProvider }: HistoricalBillingTableProps) => {
  ...
  const { meses, periodosDesviacion } = useMemo(() => {
    ...
    const handleExportExcel = useCallback(async () => {
      if (!data) return;
      try {
        const nombreArchivo = `Historico_Consumo_${cloudProvider}.xlsx`;
        
        await exportHistoricalBillingToExcel(data, meses, periodosDesviacion, nombreArchivo);
      } catch (error) {
        console.error("Error al exportar a Excel:", error);
      }
    }, [data, meses, periodosDesviacion, cloudProvider]);
