...
import { AllAdvisorRecommendations, AllAdvisorRecommendationsData } from '@/interfaces/vista-advisor/advisorViewInterfaces'
import { ChevronDown, ChevronRight, Clock, History, Info, Search, X } from 'lucide-react';
import { useEffect, useMemo, useState } from 'react';
...
const getLatestCheckDetail = (rec: AllAdvisorRecommendationsData): CheckDetail | null => {
    if (!hasCheckDetails(rec)) return null
    const details = rec.check_details
    if (!details || details.length === 0) return null
    if (details.length === 1) return details[0]
    return [...details].sort((a, b) => {
        const ta = a.sync_time ? new Date(a.sync_time).getTime() : 0
        const tb = b.sync_time ? new Date(b.sync_time).getTime() : 0
        return tb - ta
    })[0]
}

const getLatestSyncTime = (rec: AllAdvisorRecommendationsData): string | undefined => {
    const latest = getLatestCheckDetail(rec)
    return latest?.sync_time ?? rec.sync_time ?? undefined
}

const getSortedCheckDetails = (rec: AllAdvisorRecommendationsData): CheckDetail[] => {
    if (!hasCheckDetails(rec)) return []
    const details = rec.check_details
...
export const AdvisorViewInfoComponent = ({ data }: AdvisorViewInfoComponentProps) => {
    ...
    const details: CheckDetail | null = useMemo(() => {
        if (!selectedRec) return null
        return getLatestCheckDetail(selectedRec)
    }, [selectedRec])

    const historyDetails: CheckDetail[] = useMemo(() => {
        if (!selectedRec) return []
        return getSortedCheckDetails(selectedRec)
    }, [selectedRec])
                                ...
                                <div key={group.category} className="border-b last:border-b-0">
                                    ...
                                    {isOpen && (
                                        <ul role="radiogroup" className="px-1 pb-2">
                                            {group.recommendations.map((rec) => {
                                                const isSelected = String(selectedCheckId) === String(rec.check_id)
                                                const latestSync = getLatestSyncTime(rec)
                                                return (
                                                    <li key={rec.check_id}>
                                                        <button
                                                            role="radio"
                                                        ...
                                                        >
                                                            ...
                                                            {latestSync && (
                                                                <div className="flex items-center gap-1 mt-1 text-xs text-muted-foreground">
                                                                    ...
                                                                    <span>{fmtDateTime(latestSync)}</span>
                                                                </div>
                                                            )}
                                                        </button>
                                                    </li>
                            ...
                            <div className="p-4 border-b space-y-1">
                                <div className="flex items-center justify-between">
                                    ...
                                    {historyDetails.length > 1 && (
                                        <button
                                            ...
                                            className="inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-xs font-medium hover:bg-muted cursor-pointer"
                                        >
                                            <History className="h-3.5 w-3.5" />
                                            Ver historial ({historyDetails.length})
                                        </button>
                                    )}
                                </div>
                                <h2 className="text-xl font-bold leading-tight">{selectedRec.name}</h2>
                                {details?.sync_time && (
                                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                        ...
                                        <span>Última sincronización: {fmtDateTime(details.sync_time)}</span>
                                    </div>
                                )}
                            </div>
                            <div className="p-4 overflow-y-auto">
                ...
                >
                    ...
                    >
                        <div className="flex items-center justify-between p-4 border-b">
                            <div>
                                <h2 className="text-lg font-bold">Historial de sincronización</h2>
                                <p className="text-sm text-muted-foreground mt-0.5">{selectedRec.name}</p>
                            </div>
                            <button
                                onClick={() => setHistoryOpen(false)}
                                className="rounded-md p-1.5 hover:bg-muted cursor-pointer"
                        ...
                        <div className="overflow-y-auto p-4 space-y-4">
                            {historyDetails.map((entry, idx) => {
                                const entrySavingsUSD = entry.categorySpecificSummary && typeof entry.categorySpecificSummary === 'object'
                                    ? (entry.categorySpecificSummary as unknown)?.costOptimizing?.estimatedMonthlySavings
                                    : undefined
                                const entrySavingsPct = entry.categorySpecificSummary && typeof entry.categorySpecificSummary === 'object'
                                    ? (entry.categorySpecificSummary as unknown)?.costOptimizing?.estimatedPercentMonthlySavings
                                    : undefined
                                const entryResSum = entry.resourcesSummary
                                const entryFlagged = entry.flaggedResources ?? []

                                return (
                                    <div key={`${entry.sync_time ?? idx}-${idx}`} className="rounded-lg border">
                                        <div className="flex items-center justify-between p-3 border-b bg-muted/30">
                                            <div className="flex items-center gap-2">
                                                <Clock className="h-4 w-4 text-muted-foreground" />
                                                <span className="text-sm font-medium">{fmtDateTime(entry.sync_time)}</span>
                                                {idx === 0 && (
                                                    <span className="px-1.5 py-0.5 text-xs rounded bg-primary/10 text-primary ring-1 ring-primary/30 font-medium">
                                                        Último
                                                    </span>
                                                )}
                                            </div>
                                        ...
                                        <div className="p-3 space-y-3">
                                            <div className="grid sm:grid-cols-2 gap-3">
                                                <div className="rounded-lg border p-2">
                                                    <div className="text-xs text-muted-foreground mb-1">Ahorro mensual estimado</div>
                                                    <div className="text-sm font-medium">{fmtCurrencyUSD(entrySavingsUSD) ?? '—'}</div>
                                                </div>
                                                <div className="rounded-lg border p-2">
                                                    <div className="text-xs text-muted-foreground mb-1">% ahorro estimado</div>
                                                    <div className="text-sm font-medium">{fmtPct(entrySavingsPct) ?? '—'}</div>
                                                </div>
                                            </div>
                                            ...
                                            {entryResSum && (
                                                <div className="flex flex-wrap gap-2">
                                                    <span className="px-2 py-1 text-xs rounded bg-muted">
                                                        Procesados: <strong>{entryResSum.resourcesProcessed ?? 0}</strong>
                                                    </span>
                                                    <span className="px-2 py-1 text-xs rounded bg-muted">
                                                        Marcados: <strong>{entryResSum.resourcesFlagged ?? 0}</strong>
                                                    </span>
                                                    <span className="px-2 py-1 text-xs rounded bg-muted">
                                                        Ignorados: <strong>{entryResSum.resourcesIgnored ?? 0}</strong>
                                                    </span>
                                                    <span className="px-2 py-1 text-xs rounded bg-muted">
                                                        Suprimidos: <strong>{entryResSum.resourcesSuppressed ?? 0}</strong>
                                                    </span>
                                                </div>
                                            )}

                                            {entryFlagged.length > 0 && (
                                                <div className="overflow-x-auto rounded border">
                                                    <table className="w-full text-sm">
                                                        <thead className="text-left text-muted-foreground border-b">
                                                            <tr>
                                                                <th className="px-3 py-1.5 text-xs">Región</th>
                                                                <th className="px-3 py-1.5 text-xs">Estado</th>
                                                                <th className="px-3 py-1.5 text-xs">ResourceId</th>
                                                                <th className="px-3 py-1.5 text-xs">Suprimido</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            {entryFlagged.map((fr, fIdx) => (
                                                                <tr key={`${fr.resourceId ?? fIdx}-${fIdx}`} className="border-b last:border-b-0">
                                                                    <td className="px-3 py-1.5 text-xs">{fr.region ?? '—'}</td>
                                                                    <td className="px-3 py-1.5 text-xs">{statusBadge(fr.status)}</td>
                                                                    <td className="px-3 py-1.5 font-mono text-xs">{fr.resourceId}</td>
                                                                    <td className="px-3 py-1.5 text-xs">{fr.isSuppressed ? 'Sí' : 'No'}</td>
                                                                </tr>
                                                            ))}
                                                        </tbody>
                                                    </table>
