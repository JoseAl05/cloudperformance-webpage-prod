...
interface PresupuestoMensualTableComponentProps {
  ...
  onDelete?: (id: number | string) => void; // Restaurado a tu original
}
...
export const PresupuestoMensualTableComponent = ({
  ...
  const getSortIcon = (field: keyof PresupuestoAnualMensual) => {
    ...
    if (sortOrder === "asc") return <ArrowUp className="w-4 h-4 text-blue-600 dark:text-blue-400" />;
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
      ...
      <div className="overflow-x-auto" style={{ minHeight: '500px', maxHeight: '600px' }}>
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-900/50 sticky top-0 z-10">
            <tr>
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase cursor-pointer" onClick={() => handleSort("nombre_centro")}>
                <div className="flex items-center gap-2">Centro de Costo {getSortIcon("nombre_centro")}</div>
              </th>
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase cursor-pointer" onClick={() => handleSort("mes")}>
                <div className="flex items-center gap-2">Mes {getSortIcon("mes")}</div>
              </th>
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase cursor-pointer" onClick={() => handleSort("anio")}>
                <div className="flex items-center gap-2">Año {getSortIcon("anio")}</div>
              </th>
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase cursor-pointer" onClick={() => handleSort("monto_mensual_asociado")}>
                <div className="flex items-center gap-2">Monto Asociado {getSortIcon("monto_mensual_asociado")}</div>
              </th>
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase cursor-pointer" onClick={() => handleSort("monto_real_mes")}>
                <div className="flex items-center gap-2">Monto Real {getSortIcon("monto_real_mes")}</div>
              </th>
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase cursor-pointer" onClick={() => handleSort("monto_forecast_mes")}>
                <div className="flex items-center gap-2">Monto Forecast {getSortIcon("monto_forecast_mes")}</div>
              </th>
              <th className="px-6 py-4 text-center text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">Acciones</th>
            </tr>
          </thead>
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {paginatedData.map((p) => (
              <tr key={`${p.id_presupuesto_anual}-${p.mes}`} className="hover:bg-blue-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center gap-2">
                    <Building2 className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                    <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{p.nombre_centro}</span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-purple-600 dark:text-purple-400" />
                    <span className="text-sm font-semibold text-purple-600 dark:text-purple-400">{MES_LETRAS[p.mes - 1]}</span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="bg-gray-100 dark:bg-gray-700 rounded-lg px-3 py-1 inline-flex">
                    <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">{p.anio}</span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center gap-2">
                    <TrendingUp className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                    <span className="text-sm font-semibold text-blue-600 dark:text-blue-400">{formatCurrency(p.monto_mensual_asociado)}</span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center gap-2">
                    <TrendingUp className="w-4 h-4 text-green-600 dark:text-green-400" />
                    <span className="text-sm font-semibold text-green-600 dark:text-green-400">{formatCurrency(p.monto_real_mes)}</span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center gap-2">
                    <TrendingUp className="w-4 h-4 text-orange-600 dark:text-orange-400" />
                    <span className="text-sm font-semibold text-orange-600 dark:text-orange-400">{formatCurrency(p.monto_forecast_mes)}</span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-center">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        // Pasamos solo el ID, idéntico a tu lógica original
      ...
      <div className="bg-gray-50 dark:bg-gray-900/50 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="inline-flex items-center px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 hover:bg-gray-50">
            ...
            <div className="flex gap-1">
              {getPageNumbers().map((page, index) => (
                page === '...' ? (
                  ...
                  <button key={page} onClick={() => handlePageChange(page as number)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${currentPage === page ? 'bg-blue-600 text-white' : 'bg-white border hover:bg-gray-50'}`}>
                    {page}
                  </button>
                )
              ))}
            </div>
            <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="inline-flex items-center px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 hover:bg-gray-50">
