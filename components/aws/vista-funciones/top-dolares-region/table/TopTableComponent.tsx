...
export const TableComponentTop = ({
  ...
  const { data, error, isLoading } = useSWR<TableDataTop[]>(
    startDateFormatted && endDateFormatted
      ? `/api/bridge/facturacion/top_facturacion/REGION?date_from=${startDateFormatted}&date_to=${endDateFormatted}`
      : null,
    fetcher
  )
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <CardTitle className="flex items-center gap-2">
              📊 Historial de facturación por región
            </CardTitle>
            <p className="text-sm text-muted-foreground mt-1">
              Detalle de facturación de servicios en cada región
            </p>
          </div>
        </div>
      </CardHeader>
