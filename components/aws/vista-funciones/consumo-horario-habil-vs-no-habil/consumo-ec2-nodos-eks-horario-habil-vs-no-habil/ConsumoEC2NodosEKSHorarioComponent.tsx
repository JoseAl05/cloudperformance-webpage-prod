...
interface ConsumoEC2NodosEKSHorarioComponentProps {
  startDate: Date,
  endDate: Date,
  instance: string
  region: string,
...
export const ConsumoEC2NodosEKSHorarioComponent = ({ startDate, endDate, region, instance }: ConsumoEC2NodosEKSHorarioComponentProps) => {

  const startDateFormatted = startDate.toISOString().replace('Z', '').slice(0, -4);
  const endDateFormatted = endDate ? endDate.toISOString().replace('Z', '').slice(0, -4) : '';

  const workingNonWorkingUsage = useSWR(
    instance ? `/api/aws/bridge/aws/ec2/eks/business-vs-offhours_usage?date_from=${startDateFormatted}&date_to=${endDateFormatted}&region=${region}&instances=${instance}` : null,
    fetcher
  )

  const workingNonWorkingUsageSummary = useSWR(
    instance ? `/api/aws/bridge/aws/ec2/eks/business-vs-offhours_summary?date_from=${startDateFormatted}&date_to=${endDateFormatted}&region=${region}&instances=${instance}` : null,
    fetcher
  )

  const workingNonWorkingUsageSummaryByResource = useSWR(
    instance ? `/api/aws/bridge/aws/ec2/eks/business-vs-offhours_summary_by_resource?date_from=${startDateFormatted}&date_to=${endDateFormatted}&region=${region}&instances=${instance}` : null,
    fetcher
  )
