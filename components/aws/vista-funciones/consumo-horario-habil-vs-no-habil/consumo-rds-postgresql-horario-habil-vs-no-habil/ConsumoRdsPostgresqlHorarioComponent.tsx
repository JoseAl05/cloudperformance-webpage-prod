...
import { TableRdsPostgresqlMetrics } from "@/components/aws/vista-funciones/consumo-horario-habil-vs-no-habil/consumo-rds-postgresql-horario-habil-vs-no-habil/table/tableComponent"
...
const needsConversion = (metric?: string) =>
  metric?.includes("FreeStorageSpace") || metric?.includes("FreeableMemory")

export const MainViewConsumoRdsPostgresqlHorario = ({ startDate, endDate, metric, instance }: ConsumoRdsPostgresqlHorarioProps) => {
  ...
  const avgDataHabil = avgStatisticsFormatted.find((s: unknown) => s.Horario === "Habil")?.average ?? "--"
  const avgDataNoHabil = avgStatisticsFormatted.find((s: unknown) => s.Horario === "No habil")?.average ?? "--"
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">
                    {avgDataHabil !== "--" ? `${avgDataHabil} ${unit}` : "--"}
                  </p>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-600">
                    {avgDataNoHabil !== "--" ? `${avgDataNoHabil} ${unit}` : "--"}
                  </p>
                  <p className="text-xs text-muted-foreground">
