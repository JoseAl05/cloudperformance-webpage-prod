...
import { TableEC2Metrics } from "@/components/aws/vista-funciones/consumo-ec2-horario-habil-vs-no-habil/table/tableComponent"
...
export const MainViewConsumoEC2Horario = ({ startDate, endDate, metric }: ConsumoEC2HorarioProps) => {
  ...
  let avgDataHabil = 0;
  let avgDataNoHabil = 0;

  const { data, error, isLoading } = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}/aws/ec2/business-vs-offhours?date_from=${startDateFormatted}&date_to=${endDateFormatted}&metric_label=${metric}`,
    fetcher
  );
  const avgStatisticsFormatted = data && data.avgStatistics ? data.avgStatistics.map((item: unknown) => {
    if (metric === 'NetworkIn Average' || metric === 'NetworkIn Maximum' || metric === 'NetworkIn Minimum' || metric === 'NetworkOut Average' || metric === 'NetworkOut Maximum' || metric === 'NetworkOut Minimum') {
      return {
        ...item,
        average: bytesToMB(item.average)
      }
    }
    return item;
  }) : [];

  console.log(avgStatisticsFormatted);
  ...
  useEffect(() => {
    ...
    const dataFormatted = data.data.map((item: unknown) => {
      if (item.MetricLabel === 'NetworkIn Average' || item.MetricLabel === 'NetworkIn Maximum' || item.MetricLabel === 'NetworkIn Minimum' || item.MetricLabel === 'NetworkOut Average' || item.MetricLabel === 'NetworkOut Maximum' || item.MetricLabel === 'NetworkOut Minimum') {
        return {
          ...item,
          Value: bytesToMB(item.Value)
        };
      }
      return item;
    })
    const times = dataFormatted.map((item: unknown) => {
      const d = new Date(item.Timestamp);
      return `${d.getUTCDate()}/${d.getUTCMonth() + 1} ${d.getUTCHours()}:00`;
    });


    const valoresHabil = dataFormatted.map((item: unknown) =>
      item.Horario === "Habil" ? item.Value : null
    );
    const valoresNoHabil = dataFormatted.map((item: unknown) =>
