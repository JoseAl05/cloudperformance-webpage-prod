...
interface UnusedAutoscalingComponentProps {
    startDate: Date;
    endDate: Date;
    instance: string;
    region: string;
...
export const UnusedAutoscalingComponent = ({ startDate, endDate, region, instance }: UnusedAutoscalingComponentProps) => {

    const startDateFormatted = startDate.toISOString().replace('Z', '').slice(0, -4);
    const endDateFormatted = endDate ? endDate.toISOString().replace('Z', '').slice(0, -4) : '';

    const allUnusedEc2Table = useSWR(
        instance ? `/api/aws/bridge/unused/ec2/autoscaling/get_ec2_table_data?date_from=${startDateFormatted}&date_to=${endDateFormatted}&region=${region}&instance_id=${instance}` : null,
        fetcher
    )
    const allUnusedEc2Cards = useSWR(
        instance ? `/api/aws/bridge/unused/ec2/autoscaling/cards_metrics_summary?date_from=${startDateFormatted}&date_to=${endDateFormatted}&region=${region}&instance_id=${instance}` : null,
        fetcher
    )
        ...
        <>
            <div className='w-full min-w-0 px-4 py-6'>
                ...
                <div className="flex items-center gap-3 my-10">
                    ...
                    <h1 className="text-3xl font-bold text-foreground">Detalle Instancias de Autoscaling Infrautilizadas</h1>
                </div>
