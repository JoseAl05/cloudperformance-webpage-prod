...
import { ServiceGroupAWS } from '@/components/aws/vista-tags/TaggedResourcesComponent';
...
interface TagsDataTableProps {
    data: ServiceGroupAWS[];
}

export const TagsDataTableAWS = ({ data }: TagsDataTableProps) => {
    ...
    useEffect(() => {
        ...
        if (isCCModalOpen && costCenters.length === 0) {
            ...
            fetch('/api/aws/bridge/mantenedor-tags/aws/local/cost-centers')
                .then(res => res.json())
                .then(responseData => {
                ...
                .catch(err => console.error("Error al obtener centros de costo AWS:", err))
                .finally(() => {
    ...
    const handleDeleteTag = async (resourceId: string, keyToDelete: string) => {
        ...
        try {
            const response = await fetch('/api/aws/bridge/mantenedor-tags/aws/local/delete', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ resource_id: resourceId, tag_key: keyToDelete })
            });
            if (!response.ok) throw new Error('Error al eliminar');
            mutate((key) => typeof key === 'string' && key.includes('/api/aws/bridge/mantenedor-tags/aws/get_resource_tags'));
        } catch (error) {
    ...
    const handleSaveTag = async () => {
        ...
        try {
            const response = await fetch('/api/aws/bridge/mantenedor-tags/aws/local/add', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleSaveCostCenter = async () => {
        ...
        try {
            const response = await fetch('/api/aws/bridge/mantenedor-tags/aws/local/assign-bulk-cc', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
            ...
            alert(`Error al vincular el presupuesto en AWS: ${errorMessage}`);
        } finally {
    ...
    const columns = useMemo<ColumnDef<typeof flattenedData[0]>[]>(() => [
        ...
        },
        {
            accessorKey: "service",
            header: "Servicio AWS",
            cell: (info) => <span className="text-sm font-semibold text-gray-800">{info.getValue() as string}</span>
        },
        {
            accessorKey: "resourceId",
            header: "ID del Recurso",
            cell: (info) => <span className="text-xs font-mono text-gray-600 break-all">{info.getValue() as string}</span>
        },
        {
                    ...
                    <div className="flex flex-wrap gap-1.5 max-w-[450px]">
                        {Object.entries(tags).flatMap(([k, v]) => {
                            ...
                            return values.map((val, idx) => (
                                <div key={`${k}-${idx}`} className={`inline-flex items-center rounded border text-[10px] shadow-sm overflow-hidden ${isLocal ? (isCC ? 'border-green-200' : 'border-purple-200') : 'border-[#FF9900]/30'}`}>
                                    <span className={`px-1.5 py-0.5 font-medium border-r flex items-center gap-1 ${isLocal ? (isCC ? 'bg-green-50 text-green-700 border-green-200' : 'bg-purple-50 text-purple-700 border-purple-200') : 'bg-[#FF9900]/10 text-[#FF9900] border-[#FF9900]/30'}`}>
            ...
            cell: ({ row }) => (
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md border border-[#FF9900]/30 bg-[#FF9900]/10 text-[#FF9900] hover:bg-[#FF9900]/20 transition-all shadow-sm"
                    >
                ...
                <CardHeader className="border-b bg-gray-50/50 flex flex-row items-center justify-between">
                    <div>
                        <CardTitle className="text-lg flex items-center gap-2 text-gray-800 font-bold">
                            <Server className="h-5 w-5 text-[#FF9900]" /> Detalle de Inventario y Etiquetas
                        </CardTitle>
                        <p className="text-xs text-muted-foreground mt-1">Gestión local de FinOps para recursos de AWS</p>
                    </div>
                    <div className="flex gap-3 text-[10px] font-bold uppercase tracking-widest bg-white p-2 rounded-lg border shadow-sm">
                        <span className="flex items-center gap-1 text-[#FF9900]"><Cloud className="h-3 w-3" /> Nube</span>
            ...
            {selectedRowIds.length > 0 && (
                <div className="fixed bottom-10 left-1/2 transform -translate-x-1/2 z-40 bg-white border border-gray-200 px-6 py-3 rounded-full flex items-center gap-6 shadow-[0_10px_40px_rgba(0,0,0,0.15)] animate-in slide-in-from-bottom-8 duration-300">
                    <span className="text-sm font-medium text-gray-700 flex items-center gap-2">
                        <span className="bg-[#FF9900]/10 text-[#FF9900] px-2.5 py-0.5 rounded-full font-bold">
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-200">
                        ...
                        <div className="p-5 space-y-4">
                            <div className="text-[10px] bg-gray-50 p-2.5 rounded font-mono text-gray-500 break-all border border-gray-200">
                                <span className="font-bold text-[#FF9900] mr-2">RECURSO:</span> 
                                {selectedResource.resourceId}
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Etiqueta (Key)</label>
                                    <input type="text" placeholder="ej. owner" value={tagKey} onChange={e => setTagKey(e.target.value)} className="w-full border border-gray-200 p-2.5 rounded-md text-sm outline-none focus:ring-2 focus:ring-[#FF9900]/30 focus:border-[#FF9900] text-gray-800 transition-all" />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Valor (Value)</label>
                                    <input type="text" placeholder="ej. cloudperformance" value={tagValue} onChange={e => setTagValue(e.target.value)} className="w-full border border-gray-200 p-2.5 rounded-md text-sm outline-none focus:ring-2 focus:ring-[#FF9900]/30 focus:border-[#FF9900] text-gray-800 transition-all" />
                                </div>
                            </div>
                        </div>
                        <div className="p-4 bg-gray-50 flex justify-end gap-3 border-t border-gray-100">
                            ...
                            <button onClick={handleSaveTag} disabled={isProcessing || !tagKey || !tagValue} className="px-5 py-2 text-xs font-bold bg-[#FF9900] text-white rounded-md hover:bg-[#E68A00] disabled:opacity-50 flex items-center gap-2 shadow-md active:scale-95 transition-all">
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-200">
                        ...
                        <div className="p-5 space-y-4">
                            <p className="text-sm text-gray-600">
                                Asignando a <strong className="text-gray-900">{ccTargetIds.length} recurso(s) de AWS</strong>.
                            </p>
