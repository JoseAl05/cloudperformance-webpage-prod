...
export interface ServiceGroupAWS {
  service: string
  resources: ResourceItemAWS[]
}

export interface ApiResponseWrapper {
  data?: ServiceGroupAWS[]
  items?: ServiceGroupAWS[]
...
export const TagsAnalysisComponentAWS = ({ startDate, endDate, services }: TagsAnalysisComponentProps) => {
  ...
  const { data, isLoading, error } = useSWR<unknown>(
    `/api/aws/bridge/mantenedor-tags/aws/get_resource_tags?date_from=${startDateFormatted}&date_to=${endDateFormatted}&service_name=${services}`,
    fetcher
  )
    ...
    return (
      <div className="w-full min-w-0 px-4 py-10">
        <MessageCard
          ...
          description="Hubo un problema de conexión con el endpoint de tags de AWS."
          tone="error"
        />
      </div>
    )
  ...
  let gruposArray: ServiceGroupAWS[] = [];
  ...
  return (
    <div className="w-full min-w-0 px-4 py-6 space-y-6">
      <div className="flex items-center gap-3 mb-6">
        <Tags className="h-7 w-7 text-[#FF9900]" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 w-full">
        ...
        <Card className="border-l-4 border-l-[#FF9900] shadow-sm flex flex-col justify-between w-full">
          <CardContent className="p-4 sm:p-5 relative">
            <div className="flex justify-between items-start mb-3">
              ...
              <Tags className="h-5 w-5 text-[#FF9900] opacity-80" />
            </div>
      ...
      <div className="w-full mt-8">
        <TagsDataTableAWS data={gruposArray} />
      </div>
