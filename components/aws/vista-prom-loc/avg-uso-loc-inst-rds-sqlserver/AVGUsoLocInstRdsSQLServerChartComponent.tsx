...
// Interfaces para respuesta general (/promedio-loc-rds-sqlserver)
...
// Interfaces para respuesta de detalle (/promedio-loc-rds-sqlserver-detalle)
...
export const AVGUsoLocInstRdsSQLServerChartComponent = ({
  startDate,
  endDate,
  instance,
  region,
  ...
  console.log('RDS SQLSERVER DEBUG - Props received:', { startDate, endDate, instance, region, metrics });
  ...
  const apiUrl = useMemo(() => {
    ...
    if (instance && instance !== '' && instance !== 'all') {
      ...
      return `${baseUrl}/db/promedio-loc-rds-sqlserver-detalle?${queryParams}`;
    } else {
      ...
      return `${baseUrl}/db/promedio-loc-rds-sqlserver?${queryParams}`;
    }
  ...
  if (apiUrl) {
    console.log('RDS sqlserver Generated API URL:', apiUrl);
    console.log('RDS sqlserver Instance parameter:', instance);
    console.log('RDS sqlserver Using endpoint:', instance && instance !== '' && instance !== 'all' ? 'DETALLE' : 'GENERAL');
  }
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center p-12">
        ...
        <span className="ml-3">Cargando métricas RDS SQLServer...</span>
      </div>
    );
  }
    ...
    return (
      <div className="text-red-500 p-4 bg-red-50 rounded-lg border border-red-200">
        ...
        <p className="text-sm mt-1">
          No se pudieron obtener las métricas de RDS SQLServer: {error.message}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-w-0">
      <AVGUsoLocInstRdsSQLServerViewUsoPorRegionComponent 
        data={processedData}
