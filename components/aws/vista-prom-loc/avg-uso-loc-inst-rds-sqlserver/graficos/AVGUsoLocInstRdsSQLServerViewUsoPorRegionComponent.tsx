...
const formatMetricValue = (metric: string, value: number) => {
  ...
  if (m.includes('memoria disponible') || m.includes('espacio de almacenamiento libre') || m.includes('bytes')) {
    return `${bytesToGB(value)} GB`;
  }
  if (m.includes('lecturas') || m.includes('escrituras')) return `${value.toFixed(2)} IOPS`;
  if (m.includes('créditos')) return `${(+value).toFixed(4).replace(/\.?0+$/, '')} créditos`;
  if (m.includes('conexiones')) {
    return `${value.toFixed(2)} conexiones`;
  }
  return value.toFixed(2);
};
...
export const AVGUsoLocInstRdsSQLServerViewUsoPorRegionComponent = ({
  data,
  allMetrics,
  isLoading,
  error,
}: Props) => {
  // Lista de métricas (únicas y ordenadas) igual que en EC2
  ...
  // Mapa región → (métrica → dato de celda) igual que en EC2
  const table = useMemo(() => {
  ...
  return (
    <div className="rounded-lg border border-border p-5">
      <div className="p-5 border-b">
        <h2 className="text-xl font-semibold tracking-tight">Uso por Región (RDS SQL Server)</h2>
