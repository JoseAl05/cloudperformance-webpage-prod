...
import { AVGUsoLocInstRdsMySQLViewUsoPorRegionComponent } from './graficos/AVGUsoLocInstRdsMySQLViewUsoPorRegionComponent';
...
// Interfaces para respuesta general (/promedio-loc-rds-mysql)
...
// Interfaces para respuesta de detalle (/promedio-loc-rds-mysql-detalle)
interface MetricAverageDetail {
  metric_group?: string;           // Retrocompatibilidad
  metric_groups?: string[];        // Nueva estructura
  metric_label: string;
  metric_label_original: string;
  average_value: number;
  min_value: number;
  max_value: number;
  metric_count: number;
  resource_region?: string;        // Nuevo campo
}
...
interface ResourceInfo {
  ...
  resource_region: string;
  allocated_storage_formatted: string;
  engine_plus_version: string;
  sync_time_range: {
    from: string;
    to: string;
  };
...
export const AVGUsoLocInstRdsMySQLChartComponent = ({
  startDate,
  endDate,
  instance,
  region,
  ...
  console.log('RDS MYSQL DEBUG - Props received:', { startDate, endDate, instance, region, metrics });
  ...
  const apiUrl = useMemo(() => {
    ...
    if (instance && instance !== '' && instance !== 'all') {
      ...
      return `${baseUrl}/db/promedio-loc-rds-mysql-detalle?${queryParams}`;
    } else {
      ...
      return `${baseUrl}/db/promedio-loc-rds-mysql?${queryParams}`;
    }
  ...
  if (apiUrl) {
    console.log('RDS mysql Generated API URL:', apiUrl);
    console.log('RDS mysql Instance parameter:', instance);
    console.log('RDS mysql Using endpoint:', instance && instance !== '' && instance !== 'all' ? 'DETALLE' : 'GENERAL');
  }
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center p-12">
        ...
        <span className="ml-3">Cargando métricas RDS MySQL...</span>
      </div>
    );
  }
    ...
    return (
      <div className="text-red-500 p-4 bg-red-50 rounded-lg border border-red-200">
        ...
        <p className="text-sm mt-1">
          No se pudieron obtener las métricas de RDS MySQL: {error.message}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-w-0">
      <AVGUsoLocInstRdsMySQLViewUsoPorRegionComponent 
        data={processedData}
