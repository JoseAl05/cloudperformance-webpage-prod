...
import { AVGUsoLocInstRdsPgViewUsoPorRegionComponent } from './graficos/AVGUsoLocInstRdsPgViewUsoPorRegionComponent';
...
// Interfaces para respuesta general (/promedio-loc-rds-pg)
...
// Interfaces para respuesta de detalle (/promedio-loc-rds-pg-detalle)
...
export const AVGUsoLocInstRdsPgChartComponent = ({
  startDate,
  endDate,
  instance,
  region,
  ...
  console.log('RDS DEBUG - Props received:', { startDate, endDate, instance, region, metrics });
  ...
  const apiUrl = useMemo(() => {
    ...
    if (instance && instance !== '' && instance !== 'all') {
      ...
      return `${baseUrl}/db/promedio-loc-rds-pg-detalle?${queryParams}`;
    } else {
      ...
      return `${baseUrl}/db/promedio-loc-rds-pg?${queryParams}`;
    }
  ...
  if (apiUrl) {
    console.log('RDS Generated API URL:', apiUrl);
    console.log('RDS Instance parameter:', instance);
    console.log('RDS Using endpoint:', instance && instance !== '' && instance !== 'all' ? 'DETALLE' : 'GENERAL');
  }
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center p-12">
        ...
        <span className="ml-3">Cargando métricas RDS PostgreSQL...</span>
      </div>
    );
  }
    ...
    return (
      <div className="text-red-500 p-4 bg-red-50 rounded-lg border border-red-200">
        ...
        <p className="text-sm mt-1">
          No se pudieron obtener las métricas de RDS PostgreSQL: {error.message}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-w-0">
      <AVGUsoLocInstRdsPgViewUsoPorRegionComponent 
        data={processedData}
