...
// Función para determinar el grupo de una métrica RDS
...
// Función para formatear valores según el tipo de métrica RDS
const formatMetricValue = (value: number, metricLabel: string): string => {
  if (metricLabel.includes('CPU') && !metricLabel.includes('Créditos')) {
    ...
    if (value > 1073741824) return `${(value / 1073741824).toFixed(4)} GB`;
...
const getColorByAbsoluteValue = (
  ...
  console.log(`RDS MÉTRICA: ${metricLabel}, VALOR: ${value}, POSICIÓN: ${position}, TOTAL: ${totalValues}, VALORES: [${values.join(', ')}]`);
...
export const AVGUsoLocInstRdsPgViewUsoPorRegionComponent = ({
  data,
  allMetrics,
  isLoading,
  error
    ...
    <div className="bg-white dark:bg-transparent rounded-lg shadow-lg p-6 border border-gray-200 dark:border-slate-700">
      ...
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-2">
          Uso por Región - Heatmap RDS PostgreSQL
        </h2>
        <p className="text-gray-600">
          Visualización de métricas promedio por región AWS - Instancias RDS PostgreSQL
        </p>
      </div>
...
{/*       <div className="mb-6 flex flex-wrap items-center gap-4">
        ...
        <div className="flex items-center space-x-2">
          <div className="w-4 h-4 rounded" style={{backgroundColor: 'hsl(120, 50%, 75%)'}}></div>
          <span className="text-xs text-gray-600">Óptimo</span>
        </div>
        <div className="flex items-center space-x-2">
          <div className="w-4 h-4 rounded" style={{backgroundColor: 'hsl(45, 60%, 70%)'}}></div>
          <span className="text-xs text-gray-600">Moderado</span>
        </div>
        <div className="flex items-center space-x-2">
          <div className="w-4 h-4 rounded" style={{backgroundColor: 'hsl(0, 50%, 75%)'}}></div>
          ...
          {data.map((item) => (
            <div key={item.region} className="flex border-b border-gray-100 hover:bg-gray-50/50 transition-colors duration-150">
              ...
              {allMetrics.map((metric) => {
                ...
                console.log('RDS DEBUG COLOR - metric:', metric, 'value:', value);                
