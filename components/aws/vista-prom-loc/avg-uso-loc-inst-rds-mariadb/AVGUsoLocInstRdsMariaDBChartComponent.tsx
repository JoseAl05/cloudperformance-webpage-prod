...
// Interfaces para respuesta general (/promedio-loc-rds-mariadb)
...
// Interfaces para respuesta de detalle (/promedio-loc-rds-mariadb-detalle)
...
export const AVGUsoLocInstRdsMariaDBChartComponent = ({
  startDate,
  endDate,
  instance,
  region,
  ...
  console.log('RDS MARIADB DEBUG - Props received:', { startDate, endDate, instance, region, metrics });
  ...
  const apiUrl = useMemo(() => {
    ...
    if (instance && instance !== '' && instance !== 'all') {
      ...
      return `${baseUrl}/db/promedio-loc-rds-mariadb-detalle?${queryParams}`;
    } else {
      ...
      return `${baseUrl}/db/promedio-loc-rds-mariadb?${queryParams}`;
    }
  ...
  if (apiUrl) {
    console.log('RDS mariadb Generated API URL:', apiUrl);
    console.log('RDS mariadb Instance parameter:', instance);
    console.log('RDS mariadb Using endpoint:', instance && instance !== '' && instance !== 'all' ? 'DETALLE' : 'GENERAL');
  }
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center p-12">
        ...
        <span className="ml-3">Cargando métricas RDS MariaDB...</span>
      </div>
    );
  }
    ...
    return (
      <div className="text-red-500 p-4 bg-red-50 rounded-lg border border-red-200">
        ...
        <p className="text-sm mt-1">
          No se pudieron obtener las métricas de RDS MariaDB: {error.message}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-w-0">
      <AVGUsoLocInstRdsMariaDBViewUsoPorRegionComponent 
        data={processedData}
