...
import { AVGUsoLocInstRdsOracleViewUsoPorRegionComponent } from '@/components/aws/vista-prom-loc/avg-uso-loc-inst-rds-oracle/graficos/AVGUsoLocInstRdsOracleViewUsoPorRegionComponent';
...
// Interfaces para respuesta general (/promedio-loc-rds-oracle)
...
// Interfaces para respuesta de detalle (/promedio-loc-rds-oracle-detalle)
...
export const AVGUsoLocInstRdsOracleChartComponent = ({
  ...
  const apiUrl = useMemo(() => {
    ...
    if (instance && instance !== '' && instance !== 'all') {
      ...
      return `${baseUrl}/db/promedio-loc-rds-oracle?${queryParams}`;
    }
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center p-12">
        ...
        <span className="ml-3">Cargando métricas RDS Oracle...</span>
      </div>
    );
  }
    ...
    return (
      <div className="text-red-500 p-4 bg-red-50 rounded-lg border border-red-200">
        ...
        <p className="text-sm mt-1">
          No se pudieron obtener las métricas de RDS Oracle: {error.message}
        </p>
      </div>
    );
  }
