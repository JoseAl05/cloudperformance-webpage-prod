...
import { ConsumeViewRdsPgCpuMetrics } from '@/interfaces/vista-consumos/rdsPgConsumeViewInterfaces';

interface RdsConsumeViewUsageCpuComponentProps {
  data: ConsumeViewRdsPgCpuMetrics[] | null;
}
...
export const RdsConsumeViewUsageCpuComponent = ({ data }: RdsConsumeViewUsageCpuComponentProps) => {
  ...
  const { totalData, usedData, unusedData, yMaxRounded } = useMemo(() => {
    ...
    const totalData: [string, number][] = sortedData.map(item => [item.timestamp, item.total_cpu.toFixed(2)]);
    const usedData: [string, number][] = sortedData.map(item => [item.timestamp, item.used_cpu.toFixed(2)]);
    const unusedData: [string, number][] = sortedData.map(item => [item.timestamp, item.unused_cpu.toFixed(2)]);
