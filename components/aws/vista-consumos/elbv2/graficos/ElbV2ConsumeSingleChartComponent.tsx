...
// ... (METRIC_TRANSLATIONS se mantiene igual) ...
const METRIC_TRANSLATIONS: Record<string, string> = {
    'RequestCount Average': 'Peticiones (Requests)',
    'ProcessedBytes Average': 'Bytes Procesados',
    'NewFlowCount Average': 'Nuevos Flujos (New Flows)',
    'ActiveConnectionCount Average': 'Conexiones Activas',
    'NewConnectionCount Average': 'Nuevas Conexiones',
    'ActiveFlowCount Average': 'Flujos Activos',
    'ConsumedLCUs Average': 'LCUs Consumidos',
    'RuleEvaluations Average': 'Evaluaciones de Reglas',
    'HTTPCode_Target_5XX_Count Average': 'Errores 5XX (Target)',
    'TCP_Client_Reset_Count Average': 'Resets TCP (Cliente)',
};

interface ElbV2ConsumeSingleChartComponentProps {
    metricName: string;
    dataPoints: [string, string | number][];
    className?: string; // <--- AGREGAR ESTA PROP
}

export const ElbV2ConsumeSingleChartComponent = ({ metricName, dataPoints, className }: ElbV2ConsumeSingleChartComponentProps) => {
    // ... (Hooks y lógica de colores se mantienen igual) ...
    ...
    const getUnitLabel = (name: string) => {
        ...
        if (lowerName.includes('lcus')) return 'LCUs';
        return 'Count';
    };
    ...
    const option = useMemo(() => {
        // ... (Configuración de ECharts se mantiene igual) ...
