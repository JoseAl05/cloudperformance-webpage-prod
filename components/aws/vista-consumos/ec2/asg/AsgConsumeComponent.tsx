...
export const AsgConsumeComponent = ({
    startDate,
    endDate,
    instance,
    region,
}: AsgConsumeComponentProps) => {
    const startDateFormatted = startDate.toISOString().replace('Z', '').slice(0, -4)
    const endDateFormatted = endDate ? endDate.toISOString().replace('Z', '').slice(0, -4) : ''

    const ec2Metrics = useSWR(
        instance
            ? `/api/aws/bridge/autoscaling/consumo_ec2?date_from=${startDateFormatted}&date_to=${endDateFormatted}&region=${region}&resource=${instance}`
            : null,
        fetcher
    )
    const ec2Info = useSWR(
        instance
            ? `/api/aws/bridge/autoscaling/ec2/info?date_from=${startDateFormatted}&date_to=${endDateFormatted}&region=${region}&resource=${instance}`
            : null,
        fetcher
    )
    const ec2GlobalEfficiency = useSWR(
        instance
            ? `/api/aws/bridge/autoscaling/ec2/global_efficiency?date_from=${startDateFormatted}&date_to=${endDateFormatted}&region=${region}&resource=${instance}`
            : null,
        fetcher
    )
