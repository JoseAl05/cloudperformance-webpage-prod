...
export const NatGatewaysConsumeSingleChartComponent = ({ metricName, dataPoints }: NatGatewaysConsumeSingleChartComponentProps) => {
    ...
    const getCardTitle = (name: string) => {
        ...
        if (lowerName === 'bytesouttodestination average') return 'MBs Transferidos';
        if (lowerName === 'bytesinfromsource average') return 'MBs Recibidos';
        if (lowerName === 'activeconnectioncount average') return 'Conexiones Activas';
        if (lowerName === 'errorportallocation average') return 'Errores de Alocacion';
        return name;
    };
