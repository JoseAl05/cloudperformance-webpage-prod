...
import { RdsResourceViewHistoricInfoComponent } from '@/components/aws/vista-rds/instancias-rds-pg/info/RdsResourceViewHistoricInfoComponent';
...
interface RdsResourceViewInfoComponentProps {
    data: RdsInstanceData[] | null;
}
...
export const RdsResourceViewInfoComponent = ({ data }: RdsResourceViewInfoComponentProps) => {
    const statusColors = {
    ...
    const groupedInstances = data.reduce(
        ...
        {} as Record<string, RdsInstanceData[]>,
    )
        ...
        <div className='w-full xl:w-[22rem]'>
            <div className='space-y-8'>
                {Object.entries(groupedInstances).map(([instanceId, instances]) => {
                    // Obtener la instancia mas reciente
                    ...
                    return (
                        <div key={instanceId}>
                            <div className='absolute left-12 -top-10 text-sm font-medium text-muted-foreground bg-background px-2 py-1 rounded-md border border-border/30 shadow-sm z-0 transition-all duration-200 group-hover:border-purple-500/30 group-hover:text-purple-600'>
                                                    ...
                                                    <Dialog key={index}>
                                                        ...
                                                        <DialogContent className='max-w-2xl max-h-[80vh] sm:max-w-4xl'>
                                                            <DialogHeader>
                                                                ...
                                                                <DialogDescription>Información histórica de la instancia RDS</DialogDescription>
                                                            </DialogHeader>
                                                            <RdsResourceViewHistoricInfoComponent instances={instances} />
                                                        </DialogContent>
                                                    </Dialog>
                                                ))}
                                            </div>
                                    ...
                                    <div className='space-y-2 text-sm'>
                                        ...
                                        <div className='grid grid-cols-1 md:grid-cols-2 gap-2 p-2 rounded-lg transition-colors duration-200 hover:bg-muted/30'>
                                            <div className='flex items-center gap-2'>
                                                ...
                                                <p className='text-lg font-semibold'>{latestInstance.Total_Subnets_RDS_Postgresql}</p>
                                            </div>
                                            <div className='flex items-center gap-2'>
