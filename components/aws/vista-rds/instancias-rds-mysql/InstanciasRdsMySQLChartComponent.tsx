...
import { RdsMysqlResourceViewInfoComponent } from "@/components/aws/vista-rds/instancias-rds-mysql/info/RdsMysqlResourceViewInfoComponent";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { RdsMysqlEventsTableComponent } from "@/components/aws/vista-rds/instancias-rds-mysql/events/RdsMySQLEventsTable";
...
export const InstanciasRdsMySQLChartComponent = ({
  ...
  if (infoLoading) {
    return (
      <div className="flex justify-center items-center p-12">
        ...
        <span className="ml-3">Cargando instancias RDS MySQL...</span>
      </div>
    );
  }
  if (infoError) {
    return (
      <div className="text-red-500 p-4 bg-red-50 rounded-lg border border-red-200">
        ...
        <p className="text-sm mt-1">
          No se pudieron obtener las instancias RDS MySQL
        </p>
      </div>
    );
  }
  ...
  if (metricsLoading) {
    return (
      <div className="flex justify-center items-center p-12">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-orange-500"></div>
        <span className="ml-3">Cargando métricas de RDS MySQL {instance}...</span>
      </div>
    );
  }
  if (metricsError) {
    ...
    <div className="w-full min-w-0 px-4 py-6">
      <div className="flex flex-col xl:flex-row gap-8 min-w-0">
        ...
        <div className="w-full xl:max-w-sm min-w-0">
          <RdsMysqlResourceViewInfoComponent data={instanceData} />
        </div>
      ...
      <div className="flex flex-col gap-5 mt-10">
        <div className="flex items-center gap-3 my-5">
          <ChartBar className="h-8 w-8 text-orange-500" />
