...
interface RdsOracleCpuCreditsLineChartProps {
    data: MetricPoint[];
    title?: string;
    height?: string;
}

export const RdsOracleCpuCreditsLineChart = ({
    data,
    title = "Consumo y Balance de Créditos de CPU (Burstable)",
    height = "300px"
}: RdsOracleCpuCreditsLineChartProps) => {
    ...
    useEffect(() => {
        ...
        const series = Object.keys(grouped).map((label, index) => {
            // Colores específicos para las métricas de CPU Credits (tema Oracle naranja)
            const colorMap: Record<string, string> = {
                "Uso de Créditos de CPU (Promedio)": '#f97316', // naranja para uso (Oracle theme)
