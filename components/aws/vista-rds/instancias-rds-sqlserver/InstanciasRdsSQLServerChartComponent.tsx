...
import { RdsSQLServerResourceViewInfoComponent } from "@/components/aws/vista-rds/instancias-rds-sqlserver/info/RdsSQLServerResourceViewInfoComponent";
import { RdsSQLServerEventsTableComponent } from "@/components/aws/vista-rds/instancias-rds-sqlserver/events/RdsSQLServerEventsTable";
...
export const InstanciasRdsSQLServerChartComponent = ({
  ...
  let infoUrl = `/api/aws/bridge/db/instancias-rds-sqlserver?date_from=${startDateFormatted}&date_to=${endDateFormatted}`;
  ...
  const eventsUrl = shouldFetchEvents
    ? `/api/aws/bridge/db/instancias-rds-sqlserver-events?date_from=${startDateFormatted}&date_to=${endDateFormatted}&resource=${encodeURIComponent(instance)}`
    : null;
  ...
  if (infoLoading) {
    return (
      <div className="flex justify-center items-center p-12">
        ...
        <span className="ml-3">Cargando instancias RDS SQL Server...</span>
      </div>
    );
  }
  if (infoError) {
    return (
      <div className="text-red-500 p-4 bg-red-50 rounded-lg border border-red-200">
        ...
        <p className="text-sm mt-1">
          No se pudieron obtener las instancias RDS SQL Server
        </p>
      </div>
    );
  }
  ...
  if (metricsLoading) {
    return (
      <div className="flex justify-center items-center p-12">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
        <span className="ml-3">Cargando métricas de RDS SQL Server {instance}...</span>
      </div>
    );
  }
  if (metricsError) {
  ...
  return (
    <div className="w-full min-w-0 px-4 py-6">
      ...
      {/* Sección placeholder para futuras métricas */}
      <div className="flex flex-col gap-5 mt-10">
        ...
        {!metricsData || !metricsData.metrics_data?.length ? (
          <div className="text-center p-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
            ...
            <h3 className="text-lg font-semibold mb-2">Sin métricas en el rango</h3>
            <p className="text-gray-500">Ajusta el rango de fechas o verifica la instancia seleccionada.</p>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            {/* Tabla de Eventos RDS SQL Server */}
            <RdsSQLServerEventsTableComponent
              data={eventsData}
              startDate={startDate}
              endDate={endDate}
              instance={instance}
