...
interface RdsMariaDBIopsChartProps {
    data: MetricPoint[];
    title?: string;
    height?: string;
}

export const RdsMariaDBIopsChart = ({
    data,
    title = "Operaciones Lectura/Escritura (IOPS/seg)",
    height = "300px"
}: RdsMariaDBIopsChartProps) => {
    ...
    useEffect(() => {
        ...
        const series = Object.keys(grouped).map((label, index) => {
            // Colores específicos para IOPS (tema MariaDB)
            const colorMap: Record<string, { color: string; icon: string }> = {
                "Lecturas por Segundo (Promedio)": { color: '#f59e0b', icon: '📖' }, // ámbar para lectura
                "Escrituras por Segundo (Promedio)": { color: '#d97706', icon: '✍️' } // ámbar oscuro para escritura
            };
        ...
        const option: echarts.EChartsOption = {
            ...
            tooltip: {
                ...
                formatter: function (params: unknown) {
                    ...
                    result += `<div style="font-weight: bold; margin-bottom: 6px; color: #f59e0b;">⚡ ${new Date(params[0].value[0]).toLocaleString('es-ES')}</div>`;
        ...
        return (
            <div className="flex justify-center items-center h-96 bg-gradient-to-br from-amber-50 to-yellow-100 rounded-lg border-2 border-dashed border-amber-300">
                <div className="text-center">
                    ...
                    <p className="text-amber-600 font-medium">No hay datos de IOPS disponibles</p>
                    <p className="text-amber-500 text-sm mt-1">Verifica el rango de fechas seleccionado</p>
                </div>
            </div>
        );
    }
