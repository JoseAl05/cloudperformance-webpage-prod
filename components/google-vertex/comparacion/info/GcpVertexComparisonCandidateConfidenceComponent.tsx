...
import { GcpVertexCandidateConfidenceRow, GcpVertexModel, GcpVertexProviderKey } from '@/interfaces/vertex-comparison/gcpVertexComparisonInterfaces';
import { confidenceClasses, confidenceLabels, deltaClass, formatCurrency, formatPercent, formatSignedPercent } from '@/lib/gcpVertexComparisonFormatters';

interface GcpVertexCandidateConfidenceComponentProps {
    data: GcpVertexModel[];
...
interface CandidateGroup {
    ...
    rows: GcpVertexCandidateConfidenceRow[];
}
...
export const GcpVertexCandidateConfidenceComponent = ({ data, provider }: GcpVertexCandidateConfidenceComponentProps) => {
    const groups = useMemo<CandidateGroup[]>(() => {
        return (data || []).map((model) => {
            const rows: GcpVertexCandidateConfidenceRow[] = model[provider].candidates
                .map((candidate) => ({
            ...
            return { gcp_model_name: model.gcp_model_name, rows };
        });
    }, [data, provider]);
            ...
            <CardContent className="p-5">
                ...
                <div className="flex flex-col">
                    {groups.map((group, groupIndex) => (
                        <div key={group.gcp_model_name} className={cn('flex flex-col gap-2', groupIndex > 0 && 'mt-4 border-t border-slate-100 pt-4 dark:border-slate-800')}>
                            <div className="flex items-baseline gap-2">
                                <span className="text-xs font-bold text-slate-800 dark:text-slate-100">{group.gcp_model_name}</span>
