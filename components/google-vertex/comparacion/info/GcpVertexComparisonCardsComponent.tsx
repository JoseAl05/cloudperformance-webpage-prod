...
import { GcpVertexComparison, GcpVertexProviderKey } from '@/interfaces/vertex-comparison/gcpVertexComparisonInterfaces';
import { deltaClass, formatCurrency, formatInteger, formatPercent, formatSignedPercent, providerFullLabels } from '@/lib/gcpVertexComparisonFormatters';

interface GcpVertexSummaryCardsProps {
    data: GcpVertexComparison;
    provider: GcpVertexProviderKey;
}

export const GcpVertexSummaryCards = ({ data, provider }: GcpVertexSummaryCardsProps) => {
    const summary = data.summary;
    const providerData = data[provider];
    const providerSummary = providerData.summary;
    const projectedCost = provider === 'bedrock'
        ? (providerSummary as GcpVertexComparison['bedrock']['summary']).bedrock_projected_cost_usd
        : (providerSummary as GcpVertexComparison['foundry']['summary']).foundry_projected_cost_usd;

    const cards = useMemo(() => ([
        {
            key: 'gcp',
            label: 'Costo GCP Vertex AI',
            value: formatCurrency(summary.gcp_cost_usd),
            hint: `${formatInteger(summary.models_total)} modelos medidos`,
            icon: Cloud,
        ...
        },
        {
            key: 'endpoints',
            label: 'Endpoints',
            value: formatInteger(summary.endpoints_total),
            hint: `${formatInteger(summary.endpoints_idle)} sin consumo`,
            icon: Server,
        ...
        },
        {
            ...
            hint: providerSummary.models_unmapped > 0 ? 'Valorizados a costo GCP Vertex AI' : 'Todos con equivalente',
            icon: ShieldCheck,
