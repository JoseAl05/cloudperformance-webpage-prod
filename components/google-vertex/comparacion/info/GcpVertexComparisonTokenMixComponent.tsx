...
import { GcpVertexModel } from '@/interfaces/vertex-comparison/gcpVertexComparisonInterfaces';
import { formatCurrency, formatPercent, formatTokens } from '@/lib/gcpVertexComparisonFormatters';

interface GcpVertexTokenMixComponentProps {
    data: GcpVertexModel[];
}
...
export const GcpVertexTokenMixComponent = ({ data }: GcpVertexTokenMixComponentProps) => {
    ...
    const rows = useMemo<MixRow[]>(() => {
        return (data || [])
            .map((model) => {
                ...
                return { model: model.gcp_model_name, values, totalTokens, totalCost };
            })
