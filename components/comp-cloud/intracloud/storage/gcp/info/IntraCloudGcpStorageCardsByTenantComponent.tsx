...
interface IntraCloudGcpStorageCardsByTenantComponentProps {
...
const getMetricUnit = (metricName: string): string => {
    ...
    if (name.includes('queue_depth')) return 'Solicitudes';
    if (name.includes('size_gb') || name.includes('prov_size')) return 'GB';
    if (name.includes('request_count')) return 'Solicitudes';
    if (name.includes('object_count')) return 'Objetos';
    return '';
};
...
const MetricItem = ({ metric }: { metric: IntraCloudStorageMetricsSummary }) => {
    ...
    if (metric.metric_name.toLowerCase().includes("bytes")) {
...
export const IntraCloudGcpStorageCardsByTenantComponent = ({ data, service }: IntraCloudGcpStorageCardsByTenantComponentProps) => {
                        ...
                        >
                            ...
                            <CardContent className="pt-6 flex-1 space-y-6">
                                <div className="space-y-3">
                                    {
                                        service === 'storage_buckets' && (
                                            <div className="flex items-center gap-2 mb-2">
                                                <Activity className="w-4 h-4 text-slate-500" />
                                                <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                                                    Métricas Servicio Cloud Storage Bucket
                                                </h3>
                                            </div>
                                        )
                                    }
                                    {
                                        service === 'disks' && (
                                            <div className="flex items-center gap-2 mb-2">
                                                <Activity className="w-4 h-4 text-slate-500" />
                                                <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                                                    Métricas Servicio Persistent Disk
                                                </h3>
                                            </div>
                                        )
                                    }
