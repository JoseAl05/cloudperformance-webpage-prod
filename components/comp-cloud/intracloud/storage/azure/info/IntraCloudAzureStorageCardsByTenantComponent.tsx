...
import { IntraCloudDisks, IntraCloudDisksMetricsSummary } from '@/interfaces/vista-intracloud/storage/intraCloudDisksInterfaces';

type MetricSummary = IntraCloudStorageMetricsSummary | IntraCloudDisksMetricsSummary;
...
const getMetricUnit = (metricName: string): string => {
    ...
    if (name.includes('bytes/sec')) return 'MB/s';
...
const getServiceIcon = (serviceType: string) => {
    ...
    if (type.includes('disk')) return <Disc className="w-4 h-4 text-slate-500" />;
...
const MetricItem = ({ metric }: { metric: MetricSummary }) => {
    const unit = getMetricUnit(metric.metric_name);
    let metricValue: string | number;

    if (
        metric.metric_name.toLowerCase().includes("bytessize") ||
        metric.metric_name.toLowerCase().includes("capacity")
    ) {
        metricValue = bytesToGB(metric.value);
    } else if (metric.metric_name.toLowerCase().includes("bytes/sec")) {
        metricValue = bytesToMB(metric.value);
    } else {
...
export const IntraCloudAzureStorageCardsByTenantComponent = ({ data, service }: IntraCloudAzureStorageCardsByTenantComponentProps) => {
        ...
        <div className="w-full">
            <div className={cn("grid gap-6", colsClass)}>
                {data.map((tenant, index) => {
                    ...
                    let groupedChildServices: Record<string, MetricSummary[]> = {};
                    let sortedServices: string[] = [];
                    let resourcesCount = 0;
                    let tenantId = '';

                    if (service === 'strg_account') {
                        const storageTenant = tenant as IntraCloudStorage;
                        tenantId = storageTenant.tenant_id;
                        resourcesCount = storageTenant.resources_count;

                        metrics = storageTenant.metrics_summary.filter((m) => m.service_type === 'Storage_Account');
                        const childServicesMetrics = storageTenant.metrics_summary.filter((m) => m.service_type !== 'Storage_Account' && m.service_type !== 'disks');

                        groupedChildServices = childServicesMetrics.reduce<Record<string, MetricSummary[]>>((acc, metric) => {
                            const svc = metric.service_type || 'Unknown';
                            if (!acc[svc]) acc[svc] = [];
                            acc[svc].push(metric);
                            return acc;
                        }, {});
                        ...
                        const diskTenant = tenant as IntraCloudDisks;
                        tenantId = diskTenant.tenant_id;
                        resourcesCount = diskTenant.resources_count;

                        metrics = [];
                        const diskMetrics = diskTenant.metrics_summary.filter((m) => m.service_type === 'disks');

                        groupedChildServices = diskMetrics.reduce<Record<string, MetricSummary[]>>((acc, metric) => {
                            const svc = metric.service_type || 'Unknown';
                            if (!acc[svc]) acc[svc] = [];
                            acc[svc].push(metric);
                            return acc;
                    ...
                    sortedServices = Object.keys(groupedChildServices).sort();
