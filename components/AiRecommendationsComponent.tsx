...
// interface PastRecommendationsSectionProps {
//     resources: {
...
// const ResourceCard = ({ resource, onStatusChange, onOpenHistory, readOnly = false }: ResourceCardProps) => {
...
//                     {!readOnly && onStatusChange && onOpenHistory && (
...
// const PastRecommendationsSection = ({ resources, onStatusChange, onOpenHistory }: PastRecommendationsSectionProps) => {
//     const groupedByStatus = useMemo(() => {
//         const indexed = resources.resources.map((resource, originalIndex) => ({ resource, originalIndex }));
//         return STATUS_OPTIONS.map((status) => ({
//             status,
//             items: indexed
//                 .filter(({ resource }) => resource.execution_status === status)
//                 .sort((a, b) => b.resource.impact_matrix.savings_value - a.resource.impact_matrix.savings_value),
//         })).filter((group) => group.items.length > 0);
//     }, [resources]);

//     if (groupedByStatus.length === 0) return null;
...
//                     <h3 className="text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider ml-1">
//                         Recomendaciones en {status} del reporte de {new Date(resources.timestamp).toLocaleDateString()}
//                     </h3>
//                     {items.map(({ resource, originalIndex }) => {
//                         const resourceKey = Array.isArray(resource.resource_id)
//                             ? resource.resource_id.join('-')
//                             : resource.resource_id;
//                         return (
//                             <ResourceCard
//                                 key={`past-${status}-${resourceKey}-${originalIndex}`}
//                                 resource={resource}
//                                 onStatusChange={(newStatus, comment) => onStatusChange(originalIndex, newStatus, comment)}
//                                 onOpenHistory={() => onOpenHistory(originalIndex)}
//                             />
...
//     const handlePastStatusChange = async (
//         reportId: string,
//         resourceIndex: number,
//         newStatus: RecommendationStatus,
//         comment: string
//     ): Promise<void> => {
//         const report = reports.find((r) => r.report_id === reportId);
//         const pastBlock = report?.past_recommendations_execution;
//         const resource = pastBlock?.resources[resourceIndex];
//         if (!report || !pastBlock || !resource) return;

//         const payload: RecommendationStatusEvent = {
//             report_id: pastBlock.report_id,
//             recommendation_group_id: computeRecommendationGroupId(resource, report.cloud_provider),
//             cloud_provider: report.cloud_provider,
//             resource_type: resource.resource_type,
//             recommendation_subtype: resource.recommendation_subtype,
...
//             execution_status: newStatus,
//             status_assigned_at: new Date().toISOString(),
//             recommendation_created_at: pastBlock.timestamp,
//             comment: comment.length > 0 ? comment : undefined,
//         };
...
//                 setReports((prev) =>
//                     prev.map((r) => {
//                         if (r.report_id !== reportId || !r.past_recommendations_execution) return r;
//                         return {
//                             ...r,
//                             past_recommendations_execution: {
//                                 ...r.past_recommendations_execution,
//                                 resources: r.past_recommendations_execution.resources.map((res, idx) =>
//                                     idx === resourceIndex ? { ...res, execution_status: newStatus } : res
//                                 ),
//                             },
//                         };
...
//                                         Fecha Generación Reporte: {new Date(report.timestamp).toLocaleString()}
//                                     </span>
//                                 )}
//                             </div>
//                         </div>
...
//                     {report.past_recommendations_execution && report.past_recommendations_execution.resources && report.past_recommendations_execution.resources.length > 0 && (
//                         <PastRecommendationsSection
//                             resources={report.past_recommendations_execution}
//                             onStatusChange={(resourceIndex, newStatus, comment) =>
//                                 handlePastStatusChange(report.report_id, resourceIndex, newStatus, comment)
//                             }
//                             onOpenHistory={(resourceIndex) => {
//                                 const resource = report.past_recommendations_execution?.resources[resourceIndex];
//                                 if (!resource) return;
//                                 const recGroupId = computeRecommendationGroupId(resource, report.cloud_provider);
//                                 setHistoryDialog({ open: true, recGroupId });
//                             }}
