...
export const ServiceFilterComponent = ({
    ...
    const getDisplayText = () => {
        ...
        if (serviceArray.length === 1) {
            const found = azure_services.find((s) => s.value === serviceArray[0]);
    ...
    const handleServiceToggle = (serviceValue: string) => {
        ...
        if (serviceValue === 'all') {
            ...
            serviceArray = serviceArray.filter((r) => r !== 'all');
        ...
        setSelectedService(serviceArray.length ? serviceArray.join(',') : 'all_regions');
    };

    const selectedServiceArray = selectedService ? selectedService.split(',').filter(Boolean) : [];

    return !isServiceMultiselect ? (
                ...
                <Command>
                    ...
                    <CommandGroup className="max-h-[200px] overflow-y-auto">
                        <CommandItem value="all" onSelect={() => handleServiceToggle('all')}>
                            <Check className={`mr-2 h-4 w-4 ${selectedService === 'all' ? 'opacity-100' : 'opacity-0'}`} />
                            Todos los Servicios
                        </CommandItem>
                        {azure_service
