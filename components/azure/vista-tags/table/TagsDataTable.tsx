...
export const TagsDataTableAzure = ({ data }: TagsDataTableProps) => {
    ...
    useEffect(() => {
        ...
        if (isCCModalOpen && costCenters.length === 0) {
            ...
            fetch('/api/azure/bridge/mantenedor-tags/azure/local/cost-centers')
                .then(res => res.json())
                .then(responseData => {
                ...
                .catch(err => console.error("Error al obtener centros de costo:", err))
                .finally(() => {
    ...
    const handleSaveCostCenter = async () => {
        ...
        const payload = {
            ...
            resources: targetResources.map(r => ({
                ...
                id: r.id_resource,
                service: r.service,
                sub_service: r.sub_services?.[0] || "n/a",
                subscription_guid: r.subscription_guid,
        ...
        try {
            const response = await fetch('/api/azure/bridge/mantenedor-tags/azure/local/assign-bulk-cc', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
            ...
            header: () => (
                <div className="px-2">
                    <input
                        ...
                        className="rounded border-gray-300 text-[#0078D4] focus:ring-[#0078D4] w-4 h-4 cursor-pointer"
                        onChange={(e) => {
                    ...
                    <div className="flex flex-wrap gap-1.5 max-w-[450px]">
                        {Object.entries(tags).flatMap(([k, v]) => {
                            ...
                            return values.map((val, idx) => (
                                <div key={`${k}-${idx}`} className={`inline-flex items-center rounded border text-[10px] shadow-sm overflow-hidden ${isLocal ? (isCC ? 'border-green-200' : 'border-purple-200') : 'border-[#0078D4]/30'}`}>
                                    <span className={`px-1.5 py-0.5 font-medium border-r flex items-center gap-1 ${isLocal ? (isCC ? 'bg-green-50 text-green-700 border-green-200' : 'bg-purple-50 text-purple-700 border-purple-200') : 'bg-[#0078D4]/10 text-[#0078D4] border-[#0078D4]/30'}`}>
            ...
            cell: ({ row }) => (
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-bold rounded-md border border border-[#0078D4]/30 bg-[#0078D4]/10 text-[#0078D4] hover:bg-[#0078D4]/20 transition-all shadow-sm"
                    >
                        <Plus className="h-3.5 w-3.5" /> Tag Local
                    </button>
                </div>
            )
        }
            ...
            {selectedRowIds.length > 0 && (
                <div className="fixed bottom-10 left-1/2 transform -translate-x-1/2 z-40 bg-white border border-gray-200 px-6 py-3 rounded-full flex items-center gap-6 shadow-[0_10px_40px_rgba(0,0,0,0.15)] animate-in slide-in-from-bottom-8 duration-300">
                    <span className="text-sm font-medium text-gray-700 flex items-center gap-2">
                        <span className="bg-[#0078D4]/10 text-[#0078D4] px-2.5 py-0.5 rounded-full font-bold">
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-200">
                        ...
                        <div className="p-5 space-y-4">
                            <p className="text-sm text-gray-600">
                                Asignando a <strong className="text-gray-900">{ccTargetIds.length} recurso(s)</strong>.
                            </p>
                                ...
                                <div className="bg-amber-50 p-5 rounded-lg border border-amber-200 flex flex-col items-center text-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-amber-700 leading-relaxed">
                                            Para poder asociar un presupuesto a tus recursos de la nube, primero debes crearlo desde el <strong>Módulo de Presupuestos</strong>.
                                        </p>
                                    </div>
                                </div>
                            ) : 
                            ...
                            (
                                <>
                                    <p className="text-xs text-gray-500 mb-1">Puedes seleccionar múltiples presupuestos si el gasto es compartido:</p>
