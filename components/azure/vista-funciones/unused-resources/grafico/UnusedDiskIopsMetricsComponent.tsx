...
interface UnusedDiskIopsMetricsComponentProps {
...
export const UnusedDiskIopsMetricsComponent = ({ data }: UnusedDiskIopsMetricsComponentProps) => {
    ...
    const {
        diskIopsMetrics, anyData
    } = useMemo(() => {

        const diskIopsMetrics = toSeriesPairs(data ?? []);

        return {
            diskIopsMetrics,
            anyData: diskIopsMetrics.length > 0
        }
    }, [data])
    ...
    return (
        <Card className="w-full">
            <CardHeader>
                <CardTitle>IOPS Disco</CardTitle>
            </CardHeader>
            <CardContent>
