...
interface AzureDbWorkingNonWorkingHoursProps {
    startDate: Date;
    endDate: Date;
    subscription: string;
    region: string;
...
export const AzureDbWorkingNonWorkingHoursComponent = ({ 
    ...
    const buildParams = () => {
        ...
        if (selectedInstanceV2 && selectedInstanceV2 !== 'all_instances') params.append('resource', selectedInstanceV2);
    ...
    // Llamadas a los endpoints de Bases de Datos de Azure
    const { data: usageDataRaw, error: usageError, isLoading: usageLoading } = useSWR(
        shouldFetch ? `/api/azure/bridge/azure/consumo/working_hours_usage_db?${queryString}` : null, fetcher, { revalidateOnFocus: false }
    );

    const { data: summaryDataRaw, error: summaryError, isLoading: summaryLoading } = useSWR(
        shouldFetch ? `/api/azure/bridge/azure/consumo/working_hours_usage_summary_db?${queryString}` : null, fetcher, { revalidateOnFocus: false }
    );

    const { data: tableDataRaw, error: tableError, isLoading: tableLoading } = useSWR(
        shouldFetch ? `/api/azure/bridge/azure/consumo/working_hours_usage_by_resource_db?${queryString}` : null, fetcher, { revalidateOnFocus: false }
    );
    ...
    if (anyError) {
        return (
            <div className="w-full min-w-0 px-4 py-10 flex flex-col items-center gap-4">
                <MessageCard icon={AlertCircle} title="Error al cargar datos" description="Ocurrió un problema al obtener la información de bases de datos." tone="error" />
            </div>
        )
    }
    ...
    if (!hasData) {
        return (
            <div className="w-full min-w-0 px-4 py-6">
                <MessageCard icon={Info} title="Sin datos para mostrar" description="No encontramos métricas de bases de datos en el rango seleccionado." tone="warn" />
            </div>
        )
    }
    ...
    return (
        <div className='w-full min-w-0 px-4 py-6'>
            ...
            <div className="flex items-center gap-3 my-10">
                ...
                <h1 className="text-3xl font-bold text-foreground">Métricas Bases de Datos Azure horario hábil vs no hábil.</h1>
            </div>
            ...
            <div className="flex items-center gap-3 my-10">
                <Clock className="h-8 w-8 text-emerald-500" />
                <h1 className="text-3xl font-bold text-foreground">Detalle Bases de Datos Azure horario hábil vs no hábil.</h1>
            </div>
