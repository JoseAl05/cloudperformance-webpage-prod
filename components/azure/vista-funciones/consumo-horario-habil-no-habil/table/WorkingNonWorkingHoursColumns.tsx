...
const DetailsCell = ({ row }: { row: WorkingNonWorkingHoursUsageSummaryByResource }) => {
    ...
    return (
        <>
            ...
            <HistoryModal isOpen={isOpen} onClose={() => setIsOpen(false)} title={row.resource_name} region={row.resource_id} resourceType="" tabs={tabs} />
        </>
    );
};
                ...
                <div className="flex flex-col justify-center">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="font-semibold text-sm text-slate-700 dark:text-slate-200 max-w-[180px]" title={info.getValue() as string}>
                            {new Date(info.getValue() as string).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                        </span>
                    </div>
                </div>
            ),
