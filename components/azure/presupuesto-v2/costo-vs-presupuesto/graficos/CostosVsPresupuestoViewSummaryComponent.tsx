...
export const CostosVsPresupuestoChart = ({ data, facturacionData }: CostosVsPresupuestoChartProps) => {
    ...
    const option = useMemo(() => {
        ...
        if (facturacionRealData && facturacionRealData.length > 0) {
            legendItems.push('Facturación Real Azure');
        }
            ...
            <CardContent>
                <div className="flex items-center justify-center gap-2 mb-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                        Comparación mensual entre presupuesto asignado, costos reales, pronóstico y facturación real de Azure.
                    </p>
                </div>
                {(!data || data.length === 0) ? (
