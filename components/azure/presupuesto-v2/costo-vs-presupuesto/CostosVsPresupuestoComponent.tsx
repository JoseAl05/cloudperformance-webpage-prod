...
import { CostosVsPresupuestoChart } from "@/components/azure/presupuesto-v2/costo-vs-presupuesto/graficos/CostosVsPresupuestoViewSummaryComponent";
import ComparacionMensualTableComponent from "@/components/azure/presupuesto-v2/costo-vs-presupuesto/table/ComparacionMensualTableComponent";
...
export const CostosVsPresupuestoComponentAzure = ({ anio, cloud, centroDeCosto }: CentroDeCostoComponentProps) => {
    ...
    const cargarFacturacion = async () => {
      ...
      for (const presupuesto of dataPresupuesto) {
        ...
        try {
          ...
          console.error("Error facturación centro costo Azure", presupuesto.id_centro_costo, e);
    ...
    <>
      <div className="space-y-8 p-4">
        <div className="flex items-center gap-3 my-5">
          ...
          <h1 className="text-3xl font-bold text-foreground">Presupuestos {anio} - Azure</h1>
        </div>
          ...
          <Card className="border-l-4 border-l-purple-500 shadow-lg rounded-2xl">
            <CardContent className="p-6 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Consumo total de la cuenta Azure</p>
              </div>
          ...
          return (
            <div key={presupuesto.id_presupuesto_anual} className="p-4 border rounded-xl dark:border-gray-700 bg-white dark:bg-gray-800/50 shadow-sm transition-all hover:shadow-md">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-100 dark:border-gray-700/50">
                ...
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Facturación Azure Asignada</p>
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                      ...
                      <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                        {[...(presupuesto.presupuestos_mensuales || [])].sort((a, b) => a.mes - b.mes).map((mes: PresupuestoMensual) => {
                          ...
                          const consumoAzure = facturacionMensual.find((f: FacturacionAnual) => f.month === mes.mes)?.total_cost || 0;
                          const diferencia = montoPresupuesto - consumoAzure;
                          ...
                          return (
                            <tr key={mes.mes} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                              ...
                              <td className="px-4 py-2 text-right text-red-600 dark:text-red-400 font-medium">{formatCurrency(consumoAzure)}</td>
