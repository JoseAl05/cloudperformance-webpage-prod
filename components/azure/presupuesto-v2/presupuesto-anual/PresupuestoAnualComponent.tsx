...
import { PresupuestoAnualTableComponent } from "@/components/azure/presupuesto-v2/presupuesto-anual/table/PresupuestoAnualTableComponent";
import { PresupuestoAnualFormComponent, PresupuestoAnual } from "@/components/azure/presupuesto-v2/presupuesto-anual/form/PresupuestoAnualFormComponent";
import { SessionGate } from "@/components/general_presupuesto/session/SesionGate";

export const PresupuestoAnualComponent = () => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorText = await res.text();
        let errorMsg = "Error al eliminar el presupuesto";
        try {
          const errorJson = JSON.parse(errorText);
          const extracted = errorJson.detail || errorJson.message || errorJson.error;
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">
                  Presupuestos Anuales {cloudType.toUpperCase()}
                </h1>
          ...
          <DialogContent className="!max-w-none w-[70vw] h-[90vh] overflow-y-auto p-8 rounded-2xl shadow-xl">
            <DialogHeader>
              <DialogTitle className="text-gray-900 dark:text-gray-100">
                {editingPresupuesto ? `Editar Presupuesto Anual ${cloudType.toUpperCase()}` : `Nuevo Presupuesto Anual ${cloudType.toUpperCase()}`}
              </DialogTitle>
