...
import { AzureNodeBillingTableComponent } from '@/components/azure/vista-recursos-nodos/table/AzureNodeBillingTableComponent';
import { AzureNodeInfoComponent, AzureNodeData } from './AzureNodeInfoComponent';
import { ComputeEngineBilling, ComputeEngineMetrics } from '@/interfaces/vista-compute-engine/cEInterfaces';

interface AzureNodeResourceComponentProps {
    startDate: Date;
    endDate: Date;
...
export const AzureNodeResourceComponent = ({ startDate, endDate, selectedInstanceV2 }: AzureNodeResourceComponentProps) => {
    ...
    const azureInfo = useSWR(
        shouldFetch ? `/api/azure/bridge/azure/recursos/nodos/azure-node-info?date_from=${startDateFormatted}&date_to=${endDateFormatted}&node=${selectedInstanceV2}` : null,
        fetcher
    );

    const azureMetrics = useSWR(
        shouldFetch ? `/api/azure/bridge/azure/recursos/nodos/azure-node-metrics?date_from=${startDateFormatted}&date_to=${endDateFormatted}&node=${selectedInstanceV2}` : null,
        fetcher
    );

    const azureBilling = useSWR(
        shouldFetch ? `/api/azure/bridge/azure/recursos/nodos/azure-node-billing?date_from=${startDateFormatted}&date_to=${endDateFormatted}&node=${selectedInstanceV2}` : null,
        fetcher
    );
    ...
    const infoData = isNonEmptyArray<AzureNodeData>(azureInfo.data) ? azureInfo.data : null;
    ...
    if (!shouldFetch) {
        return (
            <div className="max-w-7xl mx-auto px-6 py-8">
                <div className="text-center text-gray-500 text-lg font-medium">Selecciona un nodo en los filtros para ver su detalle.</div>
            </div>
        );
    }
    ...
    if (!infoData && !metricsData && !billingData) {
        return (
            <div className="w-full min-w-0 px-4 py-6">
                <MessageCard icon={Info} title="Sin datos" description="No hay métricas para este nodo en el rango seleccionado." tone="warn" />
            </div>
        );
    }
        ...
        <div className='w-full min-w-0 px-4 py-6'>
            <div className='flex flex-col xl:flex-row gap-8 min-w-0'>
                <div className='w-full xl:max-w-sm min-w-0'>
                    <AzureNodeInfoComponent data={infoData} />
                </div>
            ...
            <div className='flex flex-col gap-5 mt-10'>
                ...
                <AzureNodeBillingTableComponent data={billingData || []} />
            </div>
        </div>
    );
}