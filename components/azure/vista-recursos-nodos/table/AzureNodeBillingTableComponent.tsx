...
import { getAzureVmBillingColumns } from './AzureVmBillingColumns';

const formatUSD = (value: number) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(value);

interface AzureNodeBillingTableComponentProps {
    data: ComputeEngineBilling[];
}

export const AzureNodeBillingTableComponent = ({ data }: AzureNodeBillingTableComponentProps) => {
    ...
    const totalUSD = useMemo(() => {
        return aggregatedData.reduce((acc, item) => acc + (item.cost_net_usd || 0), 0);
    }, [aggregatedData]);

    const columns = createColumns(getAzureVmBillingColumns());
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <CardTitle className="flex items-center gap-2 text-lg">
                            ...
                            Facturación de la Máquina Virtual
                        </CardTitle>
                        <p className="text-sm text-muted-foreground mt-1">
                            Desglose de costos por SKU (USD).
                        </p>
                    </div>
