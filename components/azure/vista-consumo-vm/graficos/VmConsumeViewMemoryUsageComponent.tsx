...
interface VmConsumeViewMemoryUsageComponentProps {
  data: AzureMetricsData[] | null;
  title: string;
}

export const VmConsumeViewMemoryUsageComponent = ({ data, title }: VmConsumeViewMemoryUsageComponentProps) => {
  ...
  const { totalData, usedData, unusedData, yMaxRounded } = useMemo(() => {
    ...
    const filteredData = safeData.filter(m => m.metric_name === 'Available Memory');
