...
interface VmConsumeViewCpuUsageComponentProps {
  data: AzureMetricsData[] | null;
  title: string;
  metricUnit: string;
}

export const VmConsumeViewCpuUsageComponent = ({ data, title, metricUnit }: VmConsumeViewCpuUsageComponentProps) => {
  ...
  const { totalData, usedData, unusedData, yMaxRounded } = useMemo(() => {
    ...
    const filteredData = safeData.filter(m => m.metric_name === 'Percentage CPU');
