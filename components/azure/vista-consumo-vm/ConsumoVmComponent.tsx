...
// Aquí importaremos los subcomponentes en los siguientes pasos
...
export const AzureVmMetricsComponent = ({
    ...
    // 1. Fetch Info & Resumen
    const infoUrl = `/api/azure/bridge/azure/consumo/consumo-vm/info?${baseParams}`;
    const infoData = useSWR(shouldFetch ? infoUrl : null, fetcher, { revalidateOnFocus: false });

    // 2. Fetch Eficiencia Global
    const efficiencyUrl = `/api/azure/bridge/azure/consumo/consumo-vm/global_efficiency?${baseParams}`;
    const efficiencyData = useSWR(shouldFetch ? efficiencyUrl : null, fetcher, { revalidateOnFocus: false });

    // 3. Fetch Gráficos de Tendencias
    const metricsUrl = `/api/azure/bridge/azure/consumo/consumo-vm/metricas_tendencia?${baseParams}`;
    const metricsData = useSWR(shouldFetch ? metricsUrl : null, fetcher, { revalidateOnFocus: false });

    // Gestión de Estados
    const anyLoading = infoData.isLoading || efficiencyData.isLoading || metricsData.isLoading;
    const anyError = infoData.error || efficiencyData.error || metricsData.error;
        ...
        return (
            <div className="w-full min-w-0 px-4 py-10 flex flex-col items-center gap-4">
                <MessageCard
                    icon={AlertCircle}
                    title="Error al cargar datos"
                    description="Ocurrió un problema al obtener las métricas de Azure."
                    tone="error"
                />
            </div>
        );
        ...
        return (
            <div className="w-full min-w-0 px-4 py-6">
                <MessageCard
                    icon={Info}
                    title="Sin datos para mostrar"
                    description="No se encontraron instancias ni métricas en el rango seleccionado."
                    tone="warn"
                />
            </div>
        );
    ...
    return (
        <div className="space-y-6 mt-6 px-4 w-full min-w-0">
            ...
            <div className="flex-1 space-y-6 min-w-0 overflow-hidden">
                <AzureVmConsumeChartComponent data={metricsData.data} />
            </div>

            {/* 3. Capa de Desglose (Tabla Interactiva) */}
            <AzureVmConsumeTableComponent data={infoData.data?.instancias || []} /> 
        </div>
    );
};
