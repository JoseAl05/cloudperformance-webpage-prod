...
interface TendenciaFacturacionPagoPorUsoLineChartComponentProps {
    data: FacturacionAzureData[];
}
...
export const TendenciaFacturacionPagoPorUsoLineChartComponent = ({
    data,
}: TendenciaFacturacionPagoPorUsoLineChartComponentProps) => {
    ...
    const processChartData = (rawData: FacturacionAzureData[]) => {
        if (!rawData?.length) return { dates: [] as string[], series: [] as unknown[] };
        ...
        const significantServices = Array.from(serviceMap.entries())
            .filter(([_, dateMap]) => Array.from(dateMap.values()).some((value) => value > 0))
            .sort((a, b) => {
            ...
            tooltipFormatter(params) {
                ...
                if (rest.length) {
                    ...
                    <div style="display:flex;align-items:center;gap:8px;justify-content:space-between;line-height:1.25;margin-top:6px;border-top:1px dashed #eee;padding-top:6px">
                      <div style="color:#444">+${rest.length} mas</div>
                      <div style="font-variant-numeric: tabular-nums;">
