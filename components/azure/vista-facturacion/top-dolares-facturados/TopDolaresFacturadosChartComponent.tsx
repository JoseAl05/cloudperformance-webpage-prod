...
interface TopDolaresFacturadosChartProps {
  startDate: Date
  endDate: Date
}
...
export const TopDolaresFacturadosChartComponent = ({
  startDate,
  endDate
}: TopDolaresFacturadosChartProps) => {
  ...
  const apiUrl = !startDate || !endDate
    ? null
    : `/api/azure/bridge/azure/top-facturacion?date_from=${startDateFormatted}&date_to=${endDateFormatted}`

  // Fetch del endpoint detallado (para relaciones)
  const apiUrlDetallado = !startDate || !endDate
    ? null
    : `/api/azure/bridge/azure/top-facturacion-detallado?date_from=${startDateFormatted}&date_to=${endDateFormatted}`
  ...
  const { topSubscriptions, topResourceGroups, topServices, topLocations, summary } = useMemo(() => {
    ...
    if (activeFilter.type && activeFilter.value && dataDetallado?.relationships) {
      ...
      filteredRelationships.forEach((rel: unknown) => {
        ...
        locationMap.set(rel.resource_location, (locationMap.get(rel.resource_location) || 0) + rel.total_cost_usd)
      })
  ...
  useEffect(() => {
    ...
    const options = createChartOptions('Top 5 Dólares Facturados Por Suscripciones', topSubscriptions, hasData, 'subscription')
  ...
  useEffect(() => {
    ...
    const options = createChartOptions('Top 5 Dólares Facturados Por Grupos de Recursos', topResourceGroups, hasData, 'resourceGroup')
  ...
  useEffect(() => {
    ...
    const options = createChartOptions('Top 5 Dólares Facturados Por Servicios', topServices, hasData, 'service')
  ...
  useEffect(() => {
    ...
    const options = createChartOptions('Top 5 Dólares Facturados Por Localización', topLocations, hasData, 'location')
