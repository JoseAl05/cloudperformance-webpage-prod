...
import { AzureDbCardsComponent } from './info/AzureDbCardsComponent';
import { AzureDbCPUChart } from './graficos/AzureDbCPUChart';
import { AzureDbStorageChart } from './graficos/AzureDbStorageChart';
import { AzureDbMemoryChart } from './graficos/AzureDbMemoryChart';
import { AzureDbTable } from './table/AzureDbTable';
...
export const AzureDbMetricsComponent = ({ 
    ...
    const buildParams = () => {
        ...
        if (selectedInstanceV2 && selectedInstanceV2 !== 'all_instances') params.append('resource_name', selectedInstanceV2);
    ...
    // --- Fetchers a los nuevos endpoints de Azure ---
    const { data: infoData, error: infoError, isLoading: infoLoading } = useSWR(
        shouldFetch ? `/api/azure/bridge/azure/consumo/consumo-db/info?${queryString}` : null, fetcher, { revalidateOnFocus: false }
    );

    const { data: efficiencyData, error: efficiencyError, isLoading: efficiencyLoading } = useSWR(
        shouldFetch ? `/api/azure/bridge/azure/consumo/consumo-db/global_efficiency?${queryString}` : null, fetcher, { revalidateOnFocus: false }
    );    

    const { data: cpuData, isLoading: cpuLoading } = useSWR(
        shouldFetch ? `/api/azure/bridge/azure/consumo/consumo-db/cpu_usage?${queryString}` : null, fetcher, { revalidateOnFocus: false }
    );
    
    const { data: storageData, isLoading: storageLoading } = useSWR(
        shouldFetch ? `/api/azure/bridge/azure/consumo/consumo-db/storage?${queryString}` : null, fetcher, { revalidateOnFocus: false }
    );
    
    const { data: memoryData, isLoading: memoryLoading } = useSWR(
        shouldFetch ? `/api/azure/bridge/azure/consumo/consumo-db/memory?${queryString}` : null, fetcher, { revalidateOnFocus: false }
    );

    // Gestión de Estados Consolidados
    const anyLoading = infoLoading || efficiencyLoading || cpuLoading || storageLoading || memoryLoading;
    const anyError = infoError || efficiencyError;
    ...
    if (anyError) {
        return (
            <div className="w-full min-w-0 px-4 py-10 flex flex-col items-center gap-4">
                <MessageCard icon={AlertCircle} title="Error al cargar datos" description="No se pudieron obtener las métricas de bases de datos de Azure." tone="error" />
            </div>
        );
    }
    ...
    if (!infoData || !infoData.instancias || infoData.instancias.length === 0) {
        return (
            <div className="w-full min-w-0 px-4 py-6">
                <MessageCard icon={Info} title="Sin bases de datos" description="No se encontraron recursos de Azure Database para los filtros seleccionados." tone="warn" />
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6 mt-6 px-4 w-full min-w-0">
            {/* 1. Tarjetas de KPIs y Eficiencia */}
            <AzureDbCardsComponent 
            ...
            <div className="flex-1 space-y-6 min-w-0 overflow-hidden">
                <AzureDbCPUChart data={cpuData} />
                <AzureDbStorageChart data={storageData} />
                <AzureDbMemoryChart data={memoryData} />
            </div>

            {/* 3. Tabla Interactiva y Modal */}
            <AzureDbTable data={infoData?.instancias || []} />
        </div>
    );
};