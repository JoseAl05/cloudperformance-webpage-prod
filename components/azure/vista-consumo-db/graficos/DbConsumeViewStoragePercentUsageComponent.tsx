...
interface DbConsumeViewStoragePercentUsageComponentProps {
  data: AzureDbMetricsData[] | null;
  title: string;
  metricUnit: string;
}

export const DbConsumeViewStoragePercentUsageComponent = ({ data, title, metricUnit }: DbConsumeViewStoragePercentUsageComponentProps) => {
  ...
  const { totalData, usedData, unusedData, yMaxRounded } = useMemo(() => {
    ...
    const filteredData = safeData.filter(m => m.metric_name === 'Storage Percent');
