...
interface AzureDbMetricsChartProps {
...
const AzureDbMetricsChart = ({ data, title, metricUnit }: AzureDbMetricsChartProps) => {
  ...
  const { totalData, usedData, unusedData, yMaxRounded } = useMemo(() => {
    ...
    const totalData: [string, number][] = sortedTimestamps.map(ts => [ts, grouped[ts].total_assigned]);
...
export const AzureDbCpuUsageComponent = ({ data }: { data: AzureDbMetricsData[] | null }) => (
  <AzureDbMetricsChart data={data} title="Uso de CPU (%)" metricUnit="%" />
);

export const AzureDbMemoryUsageComponent = ({ data }: { data: AzureDbMetricsData[] | null }) => (
  <AzureDbMetricsChart data={data} title="Uso de Memoria (%)" metricUnit="%" />
);

export const AzureDbStorageUsageComponent = ({ data }: { data: AzureDbMetricsData[] | null }) => (
  <AzureDbMetricsChart data={data} title="Almacenamiento Usado" metricUnit="GB" />
);

export const AzureDbStoragePercentComponent = ({ data }: { data: AzureDbMetricsData[] | null }) => (
  <AzureDbMetricsChart data={data} title="Almacenamiento Usado (%)" metricUnit="%" />
);
