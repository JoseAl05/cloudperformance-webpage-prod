...
interface AzureDbTrendDataPoint {
    date: string;
    timestamp: string;
    avg_value: number;
    max_value: number;
...
interface AzureDbStorageChartProps {
    data: AzureDbTrendDataPoint[] | null;
}

export const AzureDbStorageChart = ({ data }: AzureDbStorageChartProps) => {
    ...
    return (
        <Card className="w-full">
            <CardHeader>
                <CardTitle>Storage Utilizado</CardTitle>
            </CardHeader>
            <CardContent>
                <div className="flex items-center justify-center gap-2 mb-2">
                    <Info className="w-5 h-5 text-blue-500 flex-shrink-0" />
                    <p className="text-xs text-muted-foreground">
                        Las marcas de tiempo (Timestamps) están en formato <strong>UTC</strong> y se agrupan por día.
                    </p>
                </div>
                {isEmpty ? (
                    <div className="w-full h-[200px] flex items-center justify-center">
                        <p className="text-sm text-muted-foreground">
                            No hay métricas de storage disponibles.
                        </p>
                    </div>
                ) : (
