...
interface DbConsumeViewMemoryUsageComponentProps {
  data: AzureDbMetricsData[] | null;
  title: string;
  metricUnit: string;
}

export const DbConsumeViewMemoryUsageComponent = ({ data, title, metricUnit }: DbConsumeViewMemoryUsageComponentProps) => {
  ...
  const { totalData, usedData, unusedData, yMaxRounded } = useMemo(() => {
    ...
    const filteredData = safeData.filter(m => m.metric_name === 'Memory Percent');
