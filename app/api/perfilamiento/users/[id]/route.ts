    ...
    try {
        ...
        if (userEditing.role === 'admin_empresa') {
            if (userFound.client !== userEditing.client) {
                return NextResponse.json({ message: "No tienes permiso para editar usuarios fuera de tu empresa." }, { status: 403 });
            }
            if (body.role && body.role === 'admin_global') {
                 return NextResponse.json({ message: "No puedes asignar el rol 'admin_global'." }, { status: 403 });
            }
        }
        ...
        const allowedFields = [
            'email', 'username', 'role', 'is_aws', 'is_azure', 
            'user_db_aws', 'user_db_azure', 'is_active', // <-- AÑADIDO
        ...
        if (body.is_active === false && userEditing.userId === userIdToEdit) {
            return NextResponse.json({ message: "No puedes bloquear tu propia cuenta de administrador." }, { status: 403 });
        }
        ...
        const result = await usersCollection.updateOne({ _id }, { $set: updateFields });

        return NextResponse.json({ message: `Usuario ${userIdToEdit} actualizado exitosamente.` }, { status: 200 });
    ...
    try {
        ...
        if (userDeleting.role === 'admin_empresa') {
            if (userFound.client !== userDeleting.client) {
                return NextResponse.json({ message: "No tienes permiso para eliminar usuarios fuera de tu empresa." }, { status: 403 });
            }
            if (userDeleting.userId === userIdToDelete) {
                return NextResponse.json({ message: "Un administrador de empresa no puede eliminarse a sí mismo." }, { status: 403 });
            }
        }
        ...
        if (deleteResult.deletedCount > 0) {
            ...
            return NextResponse.json({ message: `Usuario ${userFound.email} eliminado y licencia actualizada.` }, { status: 200 });
        } else {
        ...
        return NextResponse.json({ message: 'Error interno del servidor durante la eliminación.' }, { status: 500 });
    }
}